 ...
 *
 * SDK Migration (2026): Migrated from legacy W object and OpenLayers to WME SDK + Turf.js
 */
...
(async function () {
  ...
  const DOWNLOAD_URL = 'https://greasyfork.org/scripts/390565-wme-hn-navpoints/code/WME%20HN%20NavPoints.user.js';
  ...
  async function createLayers() {
    ...
    // Set z-index for all layers to keep them on top (+100 above current)
    //const baseZIndex = sdk.Map.getLayerZIndex({ layerName: LAYER_HN_LINES });
    //const newZIndex = baseZIndex + 500000;
    //sdk.Map.setLayerZIndex({ layerName: LAYER_HN_LINES, zIndex: newZIndex });
    //sdk.Map.setLayerZIndex({ layerName: `${LAYER_HN_LINES}_colored`, zIndex: newZIndex });
    //sdk.Map.setLayerZIndex({ layerName: LAYER_HN_MARKERS, zIndex: newZIndex });
    //logDebug(`Set z-index to ${newZIndex} (base ${baseZIndex} + 100) for all HN NavPoints layers`);

    // Register layers in the layer switcher
    log(`Creating layer checkboxes: hnLines=${settings.hnLines}, hnNumbers=${settings.hnNumbers}`);
    ...
    log('✓ Layers created and registered in switcher');
  }

  // =====================================================================
  // GEOMETRY & GENERATION (Turf.js)
  // =====================================================================
  ...
  async function drawHNs(houseNumbersData) {
    ...
    if (lineFeatures.length > 0) {
      try {
        log(`Adding ${lineFeatures.length} line features to ${LAYER_HN_LINES}...`);
        ...
        log(`✓ Successfully added ${lineFeatures.length} line features to both layers`);
      } catch (err) {
        logError(`✗ Error adding line features:`, err);
      }
    }

    if (markerFeatures.length > 0) {
      try {
        log(`Adding ${markerFeatures.length} marker features to ${LAYER_HN_MARKERS}...`);
        sdk.Map.addFeaturesToLayer({
          features: markerFeatures,
          layerName: LAYER_HN_MARKERS,
        });
        log(`✓ Successfully added ${markerFeatures.length} marker features`);
      } catch (err) {
        logError(`✗ Error adding marker features:`, err);
      }
    }
  ...
  async function processSegmentsWithHNs() {
    ...
    // Below zoom threshold: clear and return
    if (zoomLevel < settings.disableBelowZoom) {
      logDebug('Below zoom threshold, clearing HNs');
      clearCache();
      return;
    }
    ...
    try {
      ...
      const currentSegmentIds = new Set(
        allSegments
          .filter((s) => s?.hasHouseNumbers === true)
          .map((s) => s.id)
      );
      ...
      logDebug(`Viewport: ${currentSegmentIds.size} total, +${toAdd.length} new, -${toRemove.length} left, ${unchanged} unchanged`);
  ...
  // =====================================================================
  // PHASE 3: CACHING HELPER FUNCTIONS
  // =====================================================================
  ...
  function setupEventListeners() {
    ...
    sdk.Events.on({
      ...
      eventHandler: (event) => {
        if (event.dataModelName === 'segments') {
          ...
          log('Clearing cache and refreshing HNs after segment save');
          clearCache();
    ...
    // Zoom level changed — clear cache for full refresh
    ...
    sdk.Events.on({
      ...
      eventHandler: async (checkboxInfo) => {
        log(`Layer checkbox event:`, checkboxInfo);

        if (checkboxInfo.name === 'HN NavPoints Lines') {
          settings.hnLines = checkboxInfo.checked;
          log(`HN Lines layer toggled: ${checkboxInfo.checked}`);
        ...
        if (checkboxInfo.name === 'HN NavPoints Numbers') {
          ...
          log(`HN Numbers layer toggled: ${checkboxInfo.checked}`);
          // Set layer visibility directly
    ...
    const shortcutDefs = [
      {
        ...
        cb: async () => {
          try {
            ...
            sdk.Map.setLayerVisibility({ layerName: LAYER_HN_LINES, visible: settings.hnLines });
            saveSettings();
          } catch (err) {
      ...
      },
      {
        ...
        cb: async () => {
          try {
            ...
            sdk.Map.setLayerVisibility({ layerName: LAYER_HN_MARKERS, visible: settings.hnNumbers });
            saveSettings();
          } catch (err) {
    ...
    for (const sc of shortcutDefs) {
      ...
      try {
        sdk.Shortcuts.createShortcut({
          ...
          description: sc.desc,
          callback: sc.cb,
          shortcutKeys: settings[sc.settingsKey].combo, // Use combo format
        });
      } catch (ex) {
        if (String(ex).includes('already in use')) {
          // Key conflict: register without key
          ...
          try {
            ...
            logError(`Unable to register shortcut ${sc.id}:`, ex2);
          }
        } else {
